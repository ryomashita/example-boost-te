...
struct non_owning_storage {
  template <
      class T, class T_ = std::decay_t<T>,
...
struct local_storage {
  ...
  constexpr explicit local_storage(T&& t) noexcept(
      ...
      : ptr{new(&data) T_{std::forward<T>(t)}},
        del{[](mem_t& self) { reinterpret_cast<T_*>(&self)->~T_(); }},
        copy{[](mem_t& self, const void* other) -> void* {
          if constexpr (std::is_copy_constructible_v<T_>)
            return new (&self) T_{*reinterpret_cast<const T_*>(other)};
          else
            throw std::runtime_error(
                "local_storage : erased type is not copy constructible");
        }},
        move{[](mem_t& self, void* other) -> void* {
          if constexpr (std::is_move_constructible_v<T_>)
            ...
            throw std::runtime_error(
                "local_storage : erased type is not move constructible");
        }} {
...
struct sbo_storage {
  ...
  struct type_fits
      : std::integral_constant<bool, sizeof(T_) <= Size &&
                                         Alignment % alignof(T_) == 0> {};
  ...
  constexpr explicit sbo_storage(T&& t) noexcept(
      std::is_nothrow_constructible_v<T_, T&&>)
      : ptr{new(&data) T_{std::forward<T>(t)}},
        ...
        copy{[](mem_t& self_mem, const void* other) -> void* {
          if constexpr (std::is_copy_constructible_v<T_>)
            ...
            throw std::runtime_error(
                "sbo_storage : erased type is not copy constructible");
        }},
        move{[](mem_t& self_mem, void*& other) -> void* {
          if constexpr (std::is_move_constructible_v<T_>)
            ...
            throw std::runtime_error(
                "sbo_storage : erased type is not move constructible");
        }} {}
...
class poly : detail::poly_base,
 ...
 public:
  ...
  constexpr poly(poly const&) noexcept(
      std::is_nothrow_copy_constructible_v<TStorage>) = default;
  constexpr poly& operator=(poly const&) noexcept(
      std::is_nothrow_copy_constructible_v<TStorage>) = default;
  constexpr poly(poly&&) noexcept(
      std::is_nothrow_move_constructible_v<TStorage>) = default;
  constexpr poly& operator=(poly&&) noexcept(
      std::is_nothrow_move_constructible_v<TStorage>) = default;

 private:
  template <class T, class T_ = std::decay_t<T>, class TRequires>
  constexpr explicit poly(T&& t, const TRequires) noexcept(
      std::is_nothrow_constructible_v<T_, T&&>)
      : poly{std::forward<T>(t),
  ...
  constexpr explicit poly(T&& t, std::index_sequence<Ns...>) noexcept(
    ...
    static_assert(
        std::is_copy_constructible_v<T_> || std::is_move_constructible_v<T_>,
...
constexpr auto extends_impl(std::index_sequence<Ns...>) noexcept {
  (void(typename mappings<T, Ns + 1>::template set<decltype(get(
            mappings<I, Ns + 1>{}))>{}),
   ...);
}
